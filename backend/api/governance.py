"""
governance.py — Governance API: token reporting and standards violations.
...
from backend.models import StandardsViolation, TokenUsage, Agent
...
class CreateViolationRequest(BaseModel):
    ...
    violation_type: str  # e.g. "off_topic", "excessive_tokens", "quality_failure"
    description: str
    severity: str = "medium"  # "low" | "medium" | "high"
    reported_by: str = "standards-agent"
...
async def list_violations(
    ...
    async with SessionLocal() as db:
        ...
        if agent_name is not None:
            q = q.where(StandardsViolation.agent_id == agent_name)
        rows = (await db.execute(q)).scalars().all()
        return [_violation_dict(v) for v in rows]


@router.post("/violations", status_code=201)
async def create_violation(req: CreateViolationRequest):
    """Create a new StandardsViolation row."""
    async with SessionLocal() as db:
